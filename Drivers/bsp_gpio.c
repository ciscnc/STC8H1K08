...
void gpio_init(void) {
    // Set P1 port mode
    // P1M1=0, P1M0=0: 准双向口(带上拉电阻)，防止PWM输入引脚浮空导致捕获异常
    P1M1 &= ~(1 << 0);  // 00，模式0，准双向口(带上拉)，P1.0端口配置，用于捕获第一路PWM1的输入
    P1M0 &= ~(1 << 0);

    P1M1 &= ~(1 << 1);  // 00，模式0，准双向口(带上拉)，P1.1端口配置(备用)
	P1M0 &= ~(1 << 1);

    // P1M1=0, P1M0=0: 准双向口(带上拉电阻)，防止PWM输入引脚浮空导致捕获异常
    P1M1 &= ~(1 << 4);  // 00，模式0，准双向口(带上拉)，P1.4端口配置，用于捕获第二路PWM2的输入
    P1M0 &= ~(1 << 4);
